...
\begin{itemize}
	\item[]
		...
		\item
			Independently brought up, prototyped, and developed a real-time solution to cross-reference external audio inputs with an EEG acquisition system by writing C code on a bare-metal embedded system and bringing up a printed circuit board for a single-bit ADC. Brought latency down from the previous iteration by a factor of 10.
		\item 
			Configured, built, and ran a custom RTOS and cross-toolchain using the Yocto Project in a Linux environment, which included the configuration for a bootloader, a target Operating System, and other optimizations based on the hardware of the embedded system and the host system.
		\item 
...
\begin{itemize}
	\item[]
		...
		\item
			Utilized algorithm design principles in an agile software development team environment, to convert the code base for an aircraft landing from the high-level OO scripting language of MATLAB to C++, bringing the runtime of the simulation down by a factor of 2. Despite tight time constraints and minimal assistance, I earned the "MVP" award for saving "hundreds of hours in simulation time and greatly reducing control system tuning efforts".
		\item
			Wrote unit-tests in C++ to verify the accuracy of the simulations, and leveraged object-oriented design principles to ensure the maintainability of the software.
...
\begin{itemize}
	\item[]
		...
		\item 
			Brought up multiple iterations of printed circuit boards by designing/reading schematics, consulting datasheets, assembling/designing the PCBs, and debugging the boards using logic analyzers, oscilloscopes, and other hardware level debugging tools.
...
\begin{itemize}
	\item[]
		\header
		{
			...
			{\textbf{Spring 2020}}
		\item 
			Wrote C++ code to implement the functionality of a Linux based Operating System, including the implementation of preemptive threading, building a Linux device driver for a FAT16 file system, and writing a Linux shell.
